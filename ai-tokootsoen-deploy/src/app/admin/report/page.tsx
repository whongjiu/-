...
export default function AdminReportPage() {
  ...
  // Generate image — capture at full native resolution
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <Card>
          ...
          <Input
            value={studentName === "XXX" ? "" : studentName}
            onChange={e => handleNameChange(e.target.value)}
            placeholder="输入学生姓名（默认XXX）"
          />
        ...
        <Card>
          ...
          <div className="flex items-center gap-2">
            <button
              className="w-9 h-9 rounded-lg border border-border-light flex items-center justify-center text-text-body hover:bg-bg-hover transition-colors"
              onClick={() => setNoticeDate(shiftDate(noticeDate, -1))}
            >
            ...
            <input
              type="date"
              value={noticeDate.replace(/\./g, "-")}
              onChange={e => {
                const d = new Date(e.target.value);
                setNoticeDate(`${d.getFullYear()}.${d.getMonth() + 1}.${d.getDate()}`);
              }}
              className="flex-1 px-3 py-2 rounded-btn border border-border-light bg-bg-page text-text-body text-center font-mono"
            />
            <button
              className="w-9 h-9 rounded-lg border border-border-light flex items-center justify-center text-text-body hover:bg-bg-hover transition-colors"
              onClick={() => setNoticeDate(shiftDate(noticeDate, 1))}
      ...
      <div className="text-center">
        <button
          className="px-8 py-3 text-lg font-medium bg-danger-500 text-white rounded-btn hover:bg-rose-600 transition-colors disabled:opacity-50 shadow-lg shadow-rose-200"
          ...
          {reports.map(r => (
            <div key={r.id} className="flex items-center justify-between p-3 bg-bg-page rounded-btn">
              <div className="flex-1 min-w-0">
                <h4 className="font-medium text-text-body truncate">{r.title}</h4>
                <p className="text-xs text-text-disabled">{new Date(r.createdAt).toLocaleDateString("zh-CN")}</p>
              </div>
